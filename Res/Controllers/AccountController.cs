...
namespace Res.Controllers
{
	public class AccountController : BaseController
	{
		...
		public async Task<ActionResult> AutoLogin(string userName, string password, string returnUrl = "")
		{
			...
			switch (result)
			{
				case SignInStatus.Success:
					// if user's password is default then redirect to change password page
					if (string.CompareOrdinal(password, ThisApp.DefaultPassword) == 0)
						return RedirectToAction("ChgPwd");
		...
		public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
		{
			...
			{
				case SignInStatus.Success:
					if (string.CompareOrdinal(model.Password, ThisApp.DefaultPassword) == 0)
						return RedirectToAction("ChgPwd");
					APBplDef.ResUserBpl.SetLastLoginTime(model.UserName);
					return RedirectToLocal(returnUrl);
				case SignInStatus.LockedOut:
